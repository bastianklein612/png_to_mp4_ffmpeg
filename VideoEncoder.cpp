...
bool VideoEncoder::setUpEncoder(std::string filename, int width, int height, int fps, int bitrate) {
    frameNumber = 0;
...
bool VideoEncoder::encodeFromFolder(std::string folder, std::string prefix, int numberOfFrames) {
    std::string filename;
...
AVFrame* VideoEncoder::read_png_to_avframe(const char* filename) {
    // Open the image file
    ...
    SwsContext* sws_ctx_yuv = sws_getContext(codec_ctx->width, codec_ctx->height, AV_PIX_FMT_RGB24,
